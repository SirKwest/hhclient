...
class SearchFragment : Fragment() {
    ...
    private fun processingChangedScreenState(newState: SearchFragmentState) {
        when (newState) {
            ...
            SearchFragmentState.EmptyResults -> {Toast.makeText(context, context?.getString(R.string.no_such_vacancies), Toast.LENGTH_LONG).show()}
            SearchFragmentState.LoadingNewPageOfResults -> {Toast.makeText(context, "LoadingNewPage", Toast.LENGTH_LONG).show()}
            SearchFragmentState.NoInternetAccess -> {Toast.makeText(context, "NoInternet", Toast.LENGTH_LONG).show()}
            SearchFragmentState.RequestInProgress -> {Toast.makeText(context, "Loading", Toast.LENGTH_LONG).show()}
            SearchFragmentState.ServerError -> {Toast.makeText(context, "ServerError", Toast.LENGTH_LONG).show()}
            is SearchFragmentState.ShowingResults -> {Toast.makeText(context, context?.resources?.getQuantityString(
